...
- **Shared cache + fetcher/scanner split**: As of the multi-seed feature, `run_wallet_sync` (single-seed) and `MultiSeedRun` (multi-seed) both drive a separate fetcher actor (downloads compact blocks into a network-scoped shared cache at `data_dir/cache/<network>/blocks.sqlite`) and one scanner per seed (consumes from the shared cache, scans into per-seed wallet DB via `scan_cached_blocks`). The fetcher is the only writer to the cache and holds a cross-process advisory lock on `blocks.lock`. Scanners do NOT call `sync::run` — that would `cache.delete()` blocks other scanners still need. Each per-seed workspace has a `meta.json` recording fingerprint/birthday/etc; resume across runs is keyed on `(data_dir, network, fingerprint)` rather than the user-supplied birthday.
